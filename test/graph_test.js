...
QUnit.test('String data converted to floats', () => {
  var vm = new BitSeriesVM({
    data: ["23.00", 123.45, "84.98"]
  });

  assert.deepEqual(vm.attr('data').attr(), [23.00, 123.45, 84.98]);
});

QUnit.test('NaN data dropped from series', () => {
  var vm = new BitSeriesVM({
    data: ["23.00", NaN, "bit-graph", "84.98"]
  });

  assert.deepEqual(vm.attr('data').attr(), [23.00, 84.98]);
});

QUnit.test('Adding new values changes appropriate meta values', 13, () => {
  var vm = new BitSeriesVM({
    data: [1, 2, 3, 4, 5]
  });

  vm.bind('seriesLength', function() {
    assert.ok('seriesLength changed');
  });
  vm.bind('lowestValue', function() {
    assert.ok('lowestValue changed');
  });
  vm.bind('highestValue', function() {
    assert.ok('highestValue changed');
  });

  assert.equal(vm.attr('seriesLength'), 5, 'series length is 5');
  assert.equal(vm.attr('lowestValue'), 1, 'lowest value is 1');
  assert.equal(vm.attr('highestValue'), 5, 'highest value is 5');

  can.batch.start();
  vm.attr('data').push(0);
  can.batch.stop();

  assert.equal(vm.attr('seriesLength'), 6, 'series length changed to 6');
  assert.equal(vm.attr('lowestValue'), 0, 'lowest value changed to 0');
  assert.equal(vm.attr('highestValue'), 5, 'highest value remains 5');

  can.batch.start();
  vm.attr('data').push(6);
  can.batch.stop();

  assert.equal(vm.attr('seriesLength'), 7, 'series length changed to 7');
  assert.equal(vm.attr('lowestValue'), 0, 'lowest value remains 0');
  assert.equal(vm.attr('highestValue'), 6, 'highest value changed to 6');
});

QUnit.test('Normalized data produces correct line', 5, () => {
  ...
  vm.bind('line', function(ev, n, o) {
    assert.ok('line changed');
  });

  assert.equal(vm.attr('line'), 'M0,100L25,75L50,50L75,25L100,0');

  can.batch.start();
  vm.attr('data').push(5);
  can.batch.stop();

  assert.equal(vm.attr('line'), 'M0,100L20,80L40,60L60,40L80,20L100,0');
...
QUnit.test('Normalized data and changing scales produces correct line', 3, () => {
...
QUnit.test('Un-normalized data and changing scales produces correct line', 5, () => {
  ...
  assert.equal(vm.attr('line'), 'M0,100L12.5,75L25,50L37.5,25L50,0', 'line changes when xScale changes');
  ...
  assert.equal(vm.attr('line'), 'M0,100L12.5,87.5L25,75L37.5,62.5L50,50', 'line changes when yScale changes');  
});

QUnit.module('Bit-Graph ViewModel');

QUnit.test('changing margins updates width and height', () => {
